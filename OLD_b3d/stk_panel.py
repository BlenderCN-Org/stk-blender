...
def simpleHash(x):
    ...
    return base64.b64encode(m.digest()).decode('ascii').replace('=', '').replace('/', '_').replace('+', '_').lower()[
           0:15]
...
class StkLabelPseudoProperty(StkProperty):
    def __init__(self, id, name, default=0.0, doc="(No documentation defined for this element)", fullid="", min=None,
                 max=None):
...
class StkIntProperty(StkProperty):
    # ! @param name   User-visible name for this property
    def __init__(self, id, name, default=0, doc="(No documentation defined for this element)", fullid="", min=None,
                 max=None):
...
class StkProperyGroup(StkProperty):
    # ! A floating-point property
    def __init__(self, id, name, contextLevel, default="false", subproperties=[], fullid="",
...
class StkBoolProperty(StkProperty):
    ...
    # ! A floating-point property
    def __init__(self, id, name, contextLevel, default="false", subproperties=[], box=True, fullid="",
                 doc="(No documentation defined for this element)"):
...
class StkColorProperty(StkProperty):
    # ! A floating-point property
    def __init__(self, id, name, contextLevel, default="255 255 255", fullid="",
    ...
    def recursivelyAddProperties(self, properties, layout, obj, contextLevel):
        ...
        for id in properties.keys():
            ...
            if isinstance(curr, StkProperyGroup):
                ...
                row.operator(generateOpName("screen.stk_tglbool_", curr.fullid, curr.id), text=curr.name, icon=icon,
                             emboss=False)
                row.label(" ")  # force the operator to not maximize
                if state == "true":
                    if curr.subproperties:
                        box = layout.box()
                ...
                if id in obj:
                    ...
                    if state == "true":
                        icon = 'CHECKBOX_HLT'
                split.operator(generateOpName("screen.stk_tglbool_", curr.fullid, curr.id), text="                ",
                ...
                if curr.id in obj:
                    ...
                    for value_id in curr.values:
                        ...
                        if value_id in curr_val:
                            icon = 'CHECKBOX_HLT'
                        row.operator(generateOpName("screen.stk_set_", curr.fullid, curr.id + "_" + value_id),
                ...
                if curr_value in curr.values and curr.values[curr_value].subproperties:
                    box = layout.box()
...
class StkPanelAddonPreferences(AddonPreferences):
    ...
    def draw(self, context):
        layout = self.layout
        layout.label(
            text="The data folder contains folders named 'karts', 'tracks', 'textures', etc. Please enter an absolute path.")
