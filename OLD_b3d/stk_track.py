...
def getObjectProperty(obj, key_name, default_value):
    if obj.proxy is not None:
        try:
            return obj.proxy[key_name]
        except:
            pass
...
class WaterExporter:
    ...
    def export(self, f):
        for obj in self.m_objects:
            ...
            if height:
                s = "%s height=\"%.2f\"" % (s, float(height))
            if speed:
                s = "%s speed=\"%.2f\"" % (s, float(speed))
            if length:
                s = "%s length=\"%.2f\"" % (s, float(length))
            if lAnim:
                f.write("%s>\n" % s)
    ...
    def export(self, f):
        ...
        for obj in self.m_objects:
            ...
            if not item_type:
                ...
                if item_type == "GHERRING":
                    item_type = "banana"
                if item_type == "RHERRING":
                    item_type = "item"
                if item_type == "YHERRING":
                    item_type = "big-nitro"
                if item_type == "SHERRING":
                    item_type = "small-nitro"
            else:
                if item_type == "nitro-big":
                    item_type = "big-nitro"
                if item_type == "nitro_big":
                    item_type = "big-nitro"
                if item_type == "nitro-small":
                    item_type = "small-nitro"
                if item_type == "nitro_small":
                    item_type = "small-nitro"
    ...
    def export(self, f):
        for obj in self.m_objects:
            try:
                ...
                flags = []
                if getObjectProperty(obj, "particle_condition", ""):
                ...
                if obj.animation_data and obj.animation_data.action and obj.animation_data.action.fcurves and \
...
class BlenderHairExporter:
    ...
    def processObject(self, object, stktype):
        ...
        if object.particle_systems is not None and len(object.particle_systems) >= 1 and \
            ...
            if object.particle_systems[0].settings.dupli_object is not None:
                # and getObjectProperty(object.particle_systems[0].settings.dupli_object, "type", "") == "object":
                self.m_objects.append(object)
            else:
            ...
            for particleSystem in obj.particle_systems:
                ...
                for particle in particleSystem.particles:
                    ...
                    if duplicated_obj.proxy is not None and duplicated_obj.proxy.library is not None:
                        ...
                        f.write(
                            '  <object type="animation" %s interaction="ghost" model="%s.b3d" skeletal-animation="false"></object>\n'
    ...
    def export(self, f):
        for obj in self.m_objects:
            try:
                ...
                conditions_string = ""
                if getObjectProperty(obj, "sfx_conditions", ""):
        ...
        for obj in self.m_objects:
            try:
                ...
                if trigger_type == "point":
                    f.write(
                        '  <object type="action-trigger" trigger-type="point" id=\"%s\" action="%s" distance="%s" %s/>\n' %
                        (obj.name,
                    ...
                    f.write(
                        ...
                        (getObjectProperty(obj, "action", ""), obj.location[0], obj.location[2], obj.location[1], radius,
                        obj.dimensions.z))
            except:
...
class StartPositionExporter:
    ...
    def export(self, f):
        ...
        for obj in self.m_objects:
            ...
            if id == "-1":
                log_warning("Invalid start position " + str(id))
...
class BillboardExporter:
    ...
    def export(self, f):
        for obj in self.m_objects:
            ...
            if face_len == 0:
                log_error("Billboard <" + getObjectProperty(obj, "name", obj.name) + "> must have at least one face")
                return
            if face_len > 1:
                log_error("Billboard <" + getObjectProperty(obj, "name", obj.name) + "> has more than ONE face")
                return

            # check the points
            if len(track_getFaces(data)[0].vertices) > 4:
                log_error("Billboard <" + getObjectProperty(obj, "name", obj.name) + "> has more than 4 points")
                return

            if len(track_getUVTextures(data)) < 1 or len(track_getUVTextures(data)[0].data) < 1:
                log_error("Billboard <" + getObjectProperty(obj, "name", obj.name) + "> has no UV texture")
                return
...
class DrivelineExporter:
    ...
    def processObject(self, obj, stktype):
        ...
        elif stktype in ["MAIN-DRIVELINE", "MAINDRIVELINE", "MAINDL"]:
    ...
    def export(self, f):
        ...
        if not self.lDrivelines:
            self.lDrivelines = [None]

        mainDriveline = self.lDrivelines[0]
        if mainDriveline is None and getSceneProperty(bpy.data.scenes[0], 'is_stk_node', 'false') != 'true' and not (
            is_arena or is_soccer):
    ...
    def convertDrivelinesAndSortEndCameras(self, lDrivelines, lSorted,
        ...
        for i in range(len(lEndCameras)):
            ...
            if isinstance(lEndCameras[i], tuple):
    ...
    def writeChecks(self, f, lChecks, mainDriveline):
        ...
        if mainDriveline:
            ...
            if activate:
                ...
                activate = ""
                log_warning(
                    "Warning : the main driveline does not activate any checkline. Lap counting and kart rescue will not work correctly.")
        else:
    ...
    def writeCannon(self, f, cannon):
        ...
        if not endSegmentName or endSegmentName not in bpy.data.objects:
...
class Driveline:
    def __init__(self, driveline, is_main):
        ...
        self.strict_lap = convertTextToYN(getObjectProperty(driveline, "strict_lapline", "N")) == "Y"
    ...
    def defineStartVertex(self):
        ...
        if len(self.lStart) != 2:
            log_error(
                "Driveline '%s' is incorrectly formed, cannot find the two 'antennas' that indicate where the driveline starts." % self.name)
    ...
    def convertToLists(self):
        ...
        while count < max_count:
            ...
            if len(next_left) != 1 and not warning_printed:
                ...
                for i in range(len(next_left)):
                    ...
                    print("Successor %d: %f %f %f" % (i, nextco[0], nextco[1], nextco[2]))
        ...
        if count >= max_count and not warning_printed:
            log_warning("Warning, Only the first %d vertices of driveline '%s' are exported" % (max_count, self.name))
    ...
    def computeSuccessor(self, lSorted):
        (dist, driveline_index, quad_index) = self.getDistanceTo(self.end_point, lSorted)
...
class TrackExport:
    ...
    def writeAnimationWithIPO(self, f, name, obj, ipo, objectType="animation"):
        ...
        if getObjectProperty(obj, "outline", ""):
        ...
        if parent and parent.type == "ARMATURE":
            f.write("  <object id=\"%s\" type=\"%s\" %s %s>\n" % (obj.name, objectType,
                                                                  getXYZHPRString(parent), ' '.join(flags)))
        else:
            f.write("  <object id=\"%s\" type=\"%s\" %s %s>\n" % (obj.name, objectType,
                                                                  getXYZHPRString(obj), ' '.join(flags)))
    ...
    def writeLODModels(self, f, sPath, lLODModels):
        for props in lLODModels:
            ...
            f.write(
                ...
                props['distance'], props['groupname'], b3d_name, getXYZHPRString(obj),
    ...
    def writeObject(self, f, sPath, obj):
        ...
        # can not have an IPO, so no need to test this here.
            ...
            if ipo and ipo.action:
                log_warning("Movable object %s has an ipo - ipo is ignored." % obj.name)
            shape = getObjectProperty(obj, "shape", "")
            if not shape:
                log_warning("Movable object %s has no shape - box assumed!" % obj.name)
                shape = "box"
        ...
        elif interact in ["ghost", "none", "static", "reset", "explode", "flatten", "physicsonly"]:

            ipo = obj.animation_data

            # In objects with skeletal animations the actual armature (which
            # is a parent) contains the IPO. So check for this:
            if not ipo or not ipo.action or not ipo.action.fcurves or not ipo.action.fcurves:
                parent = obj.parent
                if parent:
    ...
    def writeSceneFile(self, sPath, sTrackName, exporters, lTrack, lObjects, lSun):
        ...
        for obj in lObjects:
            ...
            elif getObjectProperty(obj, "outline", ""):
                export_non_static = True
            ...
            #   export_non_static = True
                ...
                export_non_static = True
            elif interact in ["reset", "explode", "flatten"]:
                export_non_static = True
            elif getObjectProperty(obj, "on_kart_collision", ""):
                export_non_static = True
            elif getObjectProperty(obj, "if", ""):
                export_non_static = True
            ...
            if type == 'lod_model':
                ...
                if not lod_model_name:
                    lod_model_name = obj.name
                lLODModels[group_name].append(
                    {'object': obj, 'groupname': group_name, 'distance': loddistance, 'filename': lod_model_name,
                     'modifiers': True})
                ...
                if getObjectProperty(obj, "nomodifierautolod", "false") == "true":
                    ...
                    lLODModels[group_name].append(
                        {'object': obj, 'groupname': group_name, 'distance': loddistance, 'filename': lod_model_name,
                         'modifiers': True})
                    loddistance = getObjectProperty(obj, "lod_distance", 60.0)
                    lLODModels[group_name].append({'object': obj, 'groupname': group_name, 'distance': loddistance,
                                                   'filename': lod_model_name + "_mid", 'modifiers': False})
                else:
                    loddistance = getObjectProperty(obj, "lod_distance", 60.0)
                    lLODModels[group_name].append(
                        {'object': obj, 'groupname': group_name, 'distance': loddistance, 'filename': lod_model_name,
                ...
                # this object is both a model and an instance, so also add it to the list of objects,
                # where it will be exported as a LOD instance
        ...
        if lSun:
            ...
            if s:
                sSky = "%s sun-color=\"%s\"" % (sSky, s)

            s = getObjectProperty(sun, "specular", 0)
            if s:
                sSky = "%s sun-specular=\"%s\"" % (sSky, s)

            s = getObjectProperty(sun, "diffuse", 0)
            if s:
                sSky = "%s sun-diffuse=\"%s\"" % (sSky, s)

            s = getObjectProperty(sun, "ambient", 0)
            if s:
                sSky = "%s ambient=\"%s\"" % (sSky, s)
        ...
        if getSceneProperty(scene, "ambientmap", "false") == "true":
            ...
            if s:
                sphericalHarmonicsTextures.append(s)
...
""" % (texture, sphericalHarmonicsStr, hori, verti, tex_percent, sphere_percent, speed_x, speed_y))
    ...
    def __init__(self, sFilename, exportImages, exportDrivelines, exportScene, exportMaterials):
        ...
        exporters = [drivelineExporter, WaterExporter(self, sPath), ParticleEmitterExporter(), BlenderHairExporter(),
                     SoundEmitterExporter(),
                     ActionTriggerExporter(), ItemsExporter(), BillboardExporter(), LightsExporter(),
                     LightShaftExporter(),
        ...
        if not is_arena:
            is_arena = "n"
        is_arena = not (is_arena[0] == "n" or is_arena[0] == "N" or is_arena[0] == "f" or is_arena[0] == "F")

        is_soccer = getSceneProperty(scene, "soccer", "n")
        if not is_soccer:
            is_soccer = "n"
        is_soccer = not (is_soccer[0] == "n" or is_soccer[0] == "N" or is_soccer[0] == "f" or is_soccer[0] == "F")
...
def savescene_callback(sFilename, exportImages, exportDrivelines, exportScene, exportMaterials):
    ...
    TrackExport(sFilename, exportImages,
                exportDrivelines and getSceneProperty(bpy.data.scenes[0], 'is_stk_node', 'false') != 'true',
                exportScene, exportMaterials)
...
class STK_Track_Export_Operator(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        if isANode:
            if 'name' not in context.scene or not context.scene['name']:
                self.report({'ERROR'}, "Please specify a name")
                log_error("Please specify a name")
                return {'FINISHED'}
            code = context.scene['name']
        else:
            if 'code' not in context.scene or not context.scene['code']:
    ...
    def execute(self, context):
        ...
        savescene_callback(self.filepath, self.exportImages, self.exportDrivelines, self.exportScene,
