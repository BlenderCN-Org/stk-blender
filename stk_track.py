            ...
            try:
                ...
                if len(getObjectProperty(obj, "sfx_conditions", "")) > 0:
                    conditions_string = ' conditions="' + \
                        getObjectProperty(obj, "sfx_conditions", "") + '"'
...
class DrivelineExporter:
    ...
    def convertDrivelinesAndSortEndCameras(self, lDrivelines, lSorted, lEndCameras):
...
class Driveline:
    ...
    def setIsLastMain(self, first_driveline):
        ...
        for i in range(3):
            ...
            if self.lRight[-1] is None or self.lLeft[-1] is None:
                # Invalid driveline (an error message will have been printed)
                return
    ...
    def convertToLists(self):
        ...
        while count < max_count:
            ...
            for i in neighb:
                if i == self.lLeft[-2]:
                    continue    # pointing backwards
                if i == self.lRight[-1]:
                    continue    # to opposite side
                next_left.append(i)
...
class TrackExport:
    ...
    def getModelDefinitionString(self, obj):
        ...
        #    lodstring = ' instancing="true" instancing_model="' + group + '"'
            ...
            lodstring = ' lod_instance="true" lod_group="_single_lod_' + \
                getObjectProperty(obj, "name", obj.name) + '"'
        return lodstring
    ...
    def writeSceneFile(self, sPath, sTrackName, exporters, lTrack, lObjects, lSun):
        ...
        if getSceneProperty(scene, "ambientmap", "false") == "true":
            ...
            if len(sphericalHarmonicsTextures) == 6:
                sphericalHarmonicsStr = 'sh-texture="' + \
                    " ".join(sphericalHarmonicsTextures) + '"'
            else:
...
""" % (texture, sphericalHarmonicsStr, hori, verti, tex_percent, sphere_percent, speed_x, speed_y)
    ...
    def __init__(self, sFilename, exportImages, exportDrivelines, exportScene, exportMaterials):
        ...
        exporters = [
            drivelineExporter,
            WaterExporter(self, sPath),
            ParticleEmitterExporter(),
            BlenderHairExporter(),
            SoundEmitterExporter(),
            ActionTriggerExporter(),
            ItemsExporter(),
            BillboardExporter(),
            LightsExporter(),
            LightShaftExporter(),
            StartPositionExporter(),
            LibraryNodeExporter(), navmeshExporter
        ]
